 ...
 *
 */
public class WaitActions extends GenericActions {
	
	private WebDriver driver;
	private IconfigReader reader;
	private Logger oLog = LoggerActions.getLogger(WaitActions.class);

	public WaitActions(WebDriver driver,IconfigReader reader) {
		super(driver);
		this.driver = driver;
		this.reader = reader;
	...
	public void elementExistAndVisible(By locator,int timeOutInSeconds,int pollingEveryInMiliSec) {
		...
		WebDriverWait wait = getWait(timeOutInSeconds, pollingEveryInMiliSec);
		wait.until(elementLocatedBy(locator));
		new JavaScriptActions(driver).scrollIntoView(locator);
